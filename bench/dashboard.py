...
def _stale(cache, artifacts):
    ...
    try:
        # Bundled caches survive checkout timestamp changes. Custom/local stores
        # without this metadata keep the existing timestamp invalidation.
        metadata = cache.with_suffix(".artifacts.json")
        if metadata.is_file():
            import hashlib
            expected = json.loads(metadata.read_text())
            return any(expected.get(name) != hashlib.sha256((STORE / name).read_bytes()).hexdigest()
...
def _calibration_identity(model):
    """Content identity survives Git checkout and Docker timestamp changes."""
    import hashlib
    return [[name, hashlib.sha256((STORE / name).read_bytes()).hexdigest()]
            for name in dict.fromkeys((*MODEL_FILES[model], "encoder.pkl"))
